...
# REPLACE with this:
wd <- "/DATABIG/pietrobacconi/ncbi_datasets/refseq_mitogenomes/05_MCMCtree/01_AA/00_inp_data/tree_file"
setwd( wd )
...
names( calibrations ) <- names( keep_indexes )  <- names( ind_dup ) <- 
  names( nodes_dup ) <- names( tt_all ) <- c( "AA" )

# Path to tree
path_tree <- c( "AA_tree.nwk" )
