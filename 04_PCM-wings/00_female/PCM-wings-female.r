...
plot(ER, main="Equal Rates female")
dev.off()

# --- Model 2: All Rates Different (ARD) --------------------------------------
ARD <- fitDiscrete(tree, data, model = "ARD")
plot(ARD, main="All Different Rates female")
dev.off()
...
plot(loss_only, main="Loss Only female")
dev.off()
...
plot(gain_only, main="Gain Only female")
dev.off()
...
title("topright", main="Wings Ancestral Trait Female LO", font.main = 1, adj = 1, line = -1)
legend("topleft",
