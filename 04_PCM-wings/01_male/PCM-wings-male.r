...
setwd("C:/Users/Utente/OneDrive - Alma Mater Studiorum Università di Bologna/Desktop/wings/re-test-24-07/01_male")
tree <- read.tree("GBM2_clean.tre")
data <- read.csv("male_wings_recoded.csv", row.names = 1)
...
#   Loss-only -- M -> B | B -> A | M -> A from Macropterous to Apterous
#   Gain-only -- A -> B | B -> M | A -> M form Apterous to Macropterous  

# --- Model 1: Equal Rates (ER) -----------------------------------------------
ER <- fitDiscrete(tree, data, model = "ER")
plot(ER, main="Equal Rates male")
dev.off()

# --- Model 2: All Rates Different (ARD) --------------------------------------
ARD <- fitDiscrete(tree, data, model = "ARD")
plot(ARD, main="All Different Rates male")
dev.off()
...
plot(loss_only, main="Loss Only male")
dev.off()
...
plot(gain_only, main="Gain Only male")
dev.off()
...
fitER <- ace(wings, tree, model = "ER", type = "discrete")
...
title("topright", main="Wings Ancestral Trait Male ER", font.main = 1, adj = 1)
# for ARD plot 
title("topright", main="Wings Ancestral Trait Male ARD", font.main = 1, adj = 1)
