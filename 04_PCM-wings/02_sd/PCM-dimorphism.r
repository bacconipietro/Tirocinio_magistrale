...
setwd("C:/Users/Utente/OneDrive - Alma Mater Studiorum Università di Bologna/Desktop/wings/re-test-24-07/02_sd")
tree <- read.tree("GBM2_clean.tre")
data <- read.csv("dimorphism_recoded.csv", row.names = 1)
...
plot(loss_only, main = "Loss Only sd")
dev.off()
...
plot(gain_only, main = "Gain Only sd")
dev.off()
...
state_cols   <- c("Absence Wing SD" = "#d95f02", "Presence Wing SD" = "#1b9e77")
...
title("topright", main="Sex Dimorphism ancestral reconstruction Male ARD", font.main = 1, adj = 1)
legend("topleft",
        legend = c("Absence Wing SD", "Presence Wing SD"),
        pch    = 21,
        pt.bg  = state_cols,
        pt.cex = 1.5,
        bty    = "n")
