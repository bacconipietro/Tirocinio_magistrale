...
################################################################################
# Script: flip_minus_strand_sequences.sh
...
# Usage: ./flip_minus_strand_sequences.sh [fasta_files...]
#        ./flip_minus_strand_sequences.sh old/*.fasta
#        ./flip_minus_strand_sequences.sh file1.fasta file2.fasta
################################################################################
...
# Initialize log
echo "$(date '+%Y-%m-%d %H:%M:%S') - Starting minus strand sequence flipping" | tee "$LOG_FILE"
echo "---" | tee -a "$LOG_FILE"
...
process_fasta_file() {
    ...
    # Check if file contains minus strand markers (-;)
            ...
            if [[ "$line" =~ ^">" ]]; then
                ...
                if [[ -n "$current_header" ]]; then
                    if [[ "$current_header" =~ \-\; ]]; then
                        # This header has minus strand marker - flip the sequence
...
echo -e "${YELLOW}Files with minus strand markers (-;): $files_with_minus${NC}" | tee -a "$LOG_FILE"
