...
boot_cex          <- 0.55      
boot_font         <- 1         
boot_col_low  <- "darkred"    # color for low support (0)
boot_col_high <- "darkgreen"  # color for high support (100)  
...
draw_mirror <- function() {
  ...
  # Build per-node colors scaled on bootstrap value (0–100)
  boot_vals <- suppressWarnings(as.numeric(nl))
  boot_vals_scaled <- pmin(pmax(boot_vals, 0), 100) / 100  # clamp to [0,1]
  grad_palette <- colorRampPalette(c(boot_col_low, boot_col_high))(101)
  node_colors <- ifelse(
    is.na(boot_vals_scaled),
    NA,
    grad_palette[round(boot_vals_scaled * 100) + 1]
  )

  nodelabels(nl, frame = "none", cex = boot_cex, font = boot_font,
             col = node_colors, adj = c(-0.1, -0.4))
}
