...
divergent_col     <- "red"     # Color for branches that differ between trees
...
# Brackets
family_cex        <- 0.55      # Size of the family names next to the brackets
...
# Function to draw the brackets based on contiguous families
draw_family_brackets <- function(tree, side = "left") {
  ...
  par(xpd = NA) # Allow drawing in the margins
  ...
  for(i in seq_along(runs$values)) {
    ...
    # If a family only has 1 tip, give it a mini-bracket so it's visible
    if (y_start == y_end) {
      y_start <- y_start - 0.3
      y_end <- y_end + 0.3
    }
...
message("\nFinding structural differences (Divergent clades)...")
comp <- ape::comparePhylo(tree1, tree2, force.rooted = TRUE)

# -> THE TEXT EXPORT IS HERE! <-
capture.output(print(comp), file = "Divergence_Stats.txt")
message(" -> Text statistics saved to: Divergence_Stats.txt")

# Mathematically identify unique nodes
N <- length(tree1$tip.label)
unique_nodes1 <- setdiff((N + 1):(N + tree1$Nnode), comp$NODES[,1])
unique_nodes2 <- setdiff((N + 1):(N + tree2$Nnode), comp$NODES[,2])

message("Rotating trees to mirror...")
...
# Apply Red to divergent branches
left_edge_cols <- rep(base_edge_col, nrow(tree1_rotated$edge))
left_edge_cols[tree1_rotated$edge[, 2] %in% unique_nodes1] <- divergent_col

right_edge_cols <- rep(base_edge_col, nrow(tree2_rotated$edge))
right_edge_cols[tree2_rotated$edge[, 2] %in% unique_nodes2] <- divergent_col
...
draw_mirror <- function() {
  ...
  ape::plot.phylo(
    ...
    edge.width = tree_lwd, edge.color = left_edge_cols, cex = tip_cex, 
  ...
  ape::plot.phylo(
    ...
    edge.width = tree_lwd, edge.color = right_edge_cols, cex = tip_cex, 
